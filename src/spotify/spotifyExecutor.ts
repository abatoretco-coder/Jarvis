...
async function _executeSpotifyCapability(input: {
  ...
  if (request.action === 'shuffle_set') {
    const state = slotBoolean(slots, 'state');
    if (typeof state !== 'boolean') return toErrorResponse('missing_state', 'Précisez : activer ou désactiver la lecture aléatoire ?');

    const now = await spotifyWebApi.getNowPlaying();
    if (now.ok) {
      const snapshot = toPlaybackSnapshot(now.data);
      if (typeof snapshot.shuffleState === 'boolean' && snapshot.shuffleState === state) {
        return {
          status: 'success',
          tts: state ? 'La lecture aléatoire est déjà active. Je l\'avais noté.' : 'La lecture aléatoire est déjà désactivée. L\'information est consignée.',
          data: {
            no_op: true,
            reason: 'shuffle_already_set',
            state,
            registry_version: SPOTIFY_CAPABILITY_REGISTRY_VERSION,
          },
        };
    ...
    const shuffled = await spotifyWebApi.setShuffle(state, deviceId);
    if (!shuffled.ok) return toErrorResponse(shuffled.error, 'Impossible de modifier la lecture aléatoire.', { status: shuffled.status });
    return { status: 'success', tts: state ? 'Lecture aléatoire activée. Le chaos, à votre service.' : 'Lecture aléatoire désactivée. Le chaos est terminé.', data: { registry_version: SPOTIFY_CAPABILITY_REGISTRY_VERSION } };
  }

  if (request.action === 'repeat_set') {
    const modeRaw = slotString(slots, 'mode')?.toLowerCase();
    const mode = modeRaw === 'off' || modeRaw === 'track' || modeRaw === 'context' ? modeRaw : undefined;
    if (!mode) return toErrorResponse('missing_mode', 'Le mode de répétition doit être "off", "track" ou "context".');

    const now = await spotifyWebApi.getNowPlaying();
    if (now.ok) {
      const snapshot = toPlaybackSnapshot(now.data);
      if (snapshot.repeatState === mode) {
        return {
          status: 'success',
          tts: `La répétition est déjà en mode "${mode}". Je ne fais que constater.`,
          data: {
            no_op: true,
            reason: 'repeat_already_set',
            mode,
            registry_version: SPOTIFY_CAPABILITY_REGISTRY_VERSION,
          },
        };
    ...
    const repeated = await spotifyWebApi.setRepeat(mode, deviceId);
    if (!repeated.ok) return toErrorResponse(repeated.error, 'Impossible de modifier la répétition.', { status: repeated.status });
    return { status: 'success', tts: `Répétition réglée sur "${mode}". C'est noté.`, data: { registry_version: SPOTIFY_CAPABILITY_REGISTRY_VERSION } };
  }

  if (request.action === 'seek') {
    const ms = slotNumber(slots, 'position_ms') ?? ((slotNumber(slots, 'position_seconds') ?? 0) * 1000);
    if (!ms || ms < 0) return toErrorResponse('missing_position', 'Précisez la position cible (en secondes ou millisecondes).');
    const targetMs = Math.round(ms);
    const now = await spotifyWebApi.getNowPlaying();
    if (now.ok) {
      const snapshot = toPlaybackSnapshot(now.data);
      if (typeof snapshot.progressMs === 'number') {
        const deltaMs = Math.abs(snapshot.progressMs - targetMs);
        if (deltaMs <= 1000) {
          return {
            status: 'success',
            tts: 'Déjà à cette position. Bonne observation.',
            data: {
              no_op: true,
    ...
    const seeked = await spotifyWebApi.seekToPosition(targetMs, deviceId);
    if (!seeked.ok) return toErrorResponse(seeked.error, 'Impossible de se déplacer à cette position.', { status: seeked.status });
    return { status: 'success', tts: 'Position modifiée. Je vous ai épargné le déplacement.', data: { position_ms: targetMs, registry_version: SPOTIFY_CAPABILITY_REGISTRY_VERSION } };
  }
