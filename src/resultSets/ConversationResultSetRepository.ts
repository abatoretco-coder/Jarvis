...
const REFERENCE_TEXT_PATTERN = /\b(?:celui|celle|lui|elle|premier|premiere|deuxieme|second|seconde|troisieme|quatrieme|cinquieme|dernier|derniere|film|seance|cinema|parmi ceux la|lequel|laquelle|quelle heure|passe ou|c est ou|combien|coute|pitche|disponible)\b/u;
...
export class ConversationResultSetRepository {
  ...
  resolveReferenceDetailed(threadId: string, text: string): ConversationReferenceResolution {
    ...
    const focusedFollowUp = /\b(?:et\s+)?(?:lui|elle|celui la|celle la)\b|\bil passe ou\b|\bc est ou\b|\ba quelle heure\b|\bcombien (?:ca )?coute\b|\bpitche?\s+(?:le|la)\s+moi\b|\bet demain\b|\bce cinema\b|\btoujours disponible\b|\b(?:et\s+)?(?:apres|avant)\b/u.test(normalized);
